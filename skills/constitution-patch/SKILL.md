...
description: >
  ...
  error description, and correct information. Updates constitution.md, cheatsheet,
...
Apply a targeted correction to `docs/ai/constitution.md` when the user identifies
an inaccuracy. Corrections are logged so they survive full re-runs of the pipeline.
...
Read `docs/ai/constitution.md`. If it doesn't exist, abort with:
"No constitution found. Run `/constitution` first to generate one."
...
Read `docs/ai/constitution-cheatsheet.md`. If the corrected content appears in the
cheatsheet (even in condensed form), update it there too with the same annotation.
  ...
  "corrections": [
    {
      ...
      "applied_to": ["constitution.md", "cheatsheet.md", "viewer.html"]
    }
  ]
}
```
...
- Viewer update fails: warn but don't block — constitution.md is the primary artifact
